  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-8 py-12">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <StatCard label="Revenue" value={"$" + stats.revenue.toFixed(2)} color="green" />
        <StatCard label="Profit" value={"$" + stats.profit.toFixed(2)} sub={stats.revenue > 0 ? ((stats.profit / stats.revenue) * 100).toFixed(1) + "% margin" : undefined} color="yellow" />
      </div>
        ...
        <div className="rounded-xl border border-zinc-200 p-5 dark:border-zinc-700">
          <h2 className="text-lg font-semibold mb-4">
            ...
            {stats.lowStockProducts.length > 0 && (
              <span className="ml-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-red-100 text-xs text-red-700">{stats.lowStockProducts.length}</span>
            )}
          </h2>
          {stats.lowStockProducts.length === 0 ? (
            ...
            <ul className="space-y-3">
              {stats.lowStockProducts.map((p) => (
                <li key={p.id} className="flex items-center justify-between text-sm">
                  ...
                  <span className={"font-mono font-bold " + (p.stock === 0 ? "text-red-600" : "text-yellow-600")}>{p.stock} left</span>
                </li>
              ))}
            </ul>
          )}
            ...
            <ul className="space-y-3">
              {stats.recentSales.map((s: any) => (
                <li key={s.id} className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono font-bold text-green-600">${(s.total ?? 0).toFixed(2)}</span>
                </li>
              ))}
            </ul>
          )}
        ...
        <div className="rounded-xl border border-zinc-200 p-5 dark:border-zinc-700 lg:col-span-2">
          ...
          {stats.topProducts.length === 0 ? (
            ...
            <div className="space-y-3">
              {stats.topProducts.map((t: any, i: number) => {
                const maxRev = stats.topProducts[0]?._sum?.total || 1;
                const pct = ((t._sum?.total || 0) / maxRev) * 100;
                return (
                  <div key={t.productId}>
      ...
      <div className="flex gap-4">
        <Link href="/categories" className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm text-white hover:bg-blue-700">Manage Categories</Link>
        <Link href="/products" className="rounded-lg bg-purple-600 px-5 py-2.5 text-sm text-white hover:bg-purple-700">Manage Products</Link>
        <Link href="/sales" className="rounded-lg bg-green-600 px-5 py-2.5 text-sm text-white hover:bg-green-700">View Sales</Link>
      </div>
    </div>
  );
}
