...
import type { SaleWithProduct, ProductWithCategory } from "@/app/lib/db";
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowCreate(true)} className="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700">+ Record Sale</button>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <input type="text" placeholder="Search product, customer..." value={search} onChange={(e) => setSearch(e.target.value)} className="flex-1 min-w-[200px] rounded-lg border border-zinc-300 px-4 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800" />
        <select value={filterProduct} onChange={(e) => setFilterProduct(e.target.value)} className="rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800">
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Record a Sale">
        <form action={async (fd) => { await createSaleAction(fd); setShowCreate(false); }} className="space-y-4">
          <div>
            ...
            <select name="productId" required className={inputCls}>
              ...
              {activeProducts.map((p) => (<option key={p.id} value={p.id}>{p.name} — ${(p.price ?? 0).toFixed(2)} ({p.stock} in stock)</option>))}
            </select>
          </div>
          <div>
          ...
          <button type="submit" className="w-full rounded-lg bg-green-600 py-2 text-white hover:bg-green-700">Record Sale</button>
        </form>
      </Modal>

      {filtered.length === 0 ? (
        <div className="py-16 text-center"><p className="text-lg text-zinc-500">No sales recorded yet</p></div>
      ) : (
              ...
              {filtered.map((sale) => (
                <tr key={sale.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                  <td className="p-3">
                    ...
                    <p className="text-xs" style={{ color: sale.product?.category?.color ?? "#888" }}>{sale.product?.category?.name ?? ""}</p>
                  </td>
                  ...
                  <td className="p-3 text-right">
                    <form action={deleteSaleAction}><input type="hidden" name="id" value={sale.id} /><button type="submit" className="text-red-600 hover:underline">Refund</button></form>
                  </td>
                </tr>
              ))}
            </tbody>
